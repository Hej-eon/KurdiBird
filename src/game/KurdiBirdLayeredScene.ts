...
export class KurdiBirdLayeredScene extends Phaser.Scene {
  private birdBody!: Phaser.GameObjects.Ellipse;
  private birdVisual!: Phaser.GameObjects.Container;
  private bodySprite!: Phaser.GameObjects.Image;
  private wingSprite!: Phaser.GameObjects.Image;
  ...
  preload(): void {
    this.load.svg('kurdi-body-layer', 'assets/kurdi-bird-body.svg', { width: 160, height: 100 });
    this.load.svg('kurdi-wing-down-layer', 'assets/kurdi-bird-wing-down.svg', { width: 160, height: 100 });
    this.load.svg('kurdi-wing-mid-layer', 'assets/kurdi-bird-wing-mid.svg', { width: 160, height: 100 });
    this.load.svg('kurdi-wing-up-layer', 'assets/kurdi-bird-wing-up.svg', { width: 160, height: 100 });
  }
  ...
  update(time: number, delta: number): void {
    ...
    if (this.birdBody.y > GROUND_Y - BIRD_RADIUS || this.birdBody.y < BIRD_RADIUS) this.endRun();
  }
  ...
  private createBird(): void {
    ...
    this.bodySprite = this.add.image(0, 0, 'kurdi-body-layer').setDisplaySize(BIRD_WIDTH, BIRD_HEIGHT).setOrigin(0.5);
    this.wingSprite = this.add.image(0, 0, 'kurdi-wing-down-layer').setDisplaySize(BIRD_WIDTH, BIRD_HEIGHT).setOrigin(0.5);
    this.wingSprite.setDepth(1);
    this.bodySprite.setDepth(0);
    this.birdVisual.add([this.bodySprite, this.wingSprite]);
    ...
    physicsBody.setCircle(BIRD_RADIUS).setAllowGravity(true).setGravityY(GRAVITY).setCollideWorldBounds(false);
  }

  private setWingFrame(frame: number): void {
    const keys = ['kurdi-wing-down-layer', 'kurdi-wing-mid-layer', 'kurdi-wing-up-layer'];
    this.wingSprite.setTexture(keys[frame] ?? keys[0]);
  }

  private animateWing(): void {
    const token = ++this.flapToken;
    this.setWingFrame(1);
    this.time.delayedCall(65, () => token === this.flapToken && this.setWingFrame(2));
    this.time.delayedCall(135, () => token === this.flapToken && this.setWingFrame(1));
    this.time.delayedCall(210, () => token === this.flapToken && this.setWingFrame(0));
  }
  ...
  private flap(): void {
    ...
    this.tweens.add({ targets: this.birdVisual, rotation: 0.18, duration: 105, yoyo: true, ease: 'Cubic.easeOut' });
    this.tweens.add({ targets: this.birdVisual, scaleX: 1.04, scaleY: 0.97, duration: 80, yoyo: true, ease: 'Quad.easeOut' });
    this.animateWing();
  }
